...
/**
 * Generate welcome email template after successful onboarding
 */
function getWelcomeEmailTemplate(userName, email) {
  return `
    <!DOCTYPE html>
    <html lang="en">
    <head>
      <meta charset="UTF-8">
      <meta name="viewport" content="width=device-width, initial-scale=1.0">
      <title>Welcome to TechUtsav</title>
      <style>
        body { font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; background-color: #f4f4f4; margin: 0; padding: 0; }
        .container { max-width: 600px; margin: 40px auto; background: #ffffff; border-radius: 8px; overflow: hidden; box-shadow: 0 2px 8px rgba(0,0,0,0.1); }
        ...
        .content { padding: 40px 30px; color: #333333; line-height: 1.6; }
        .button { display: inline-block; padding: 14px 32px; background: linear-gradient(135deg, #4facfe 0%, #00f2fe 100%); color: white; text-decoration: none; border-radius: 6px; font-weight: 600; margin: 20px 0; transition: transform 0.2s; }
        .button:hover { transform: translateY(-2px); }
        .footer { background: #f8f9fa; padding: 20px; text-align: center; color: #666666; font-size: 12px; border-top: 1px solid #e0e0e0; }
        .feature-box { background: #f8f9fa; padding: 20px; border-radius: 8px; margin: 20px 0; }
        .feature-box h3 { color: #4facfe; margin-top: 0; }
      </style>
    </head>
    <body>
      <div class="container">
        <div class="header">
          <h1>🎉 Welcome to TechUtsav!</h1>
        </div>
        <div class="content">
          <h2>Hi ${userName},</h2>
          <p>Congratulations! Your registration is complete, and you're now part of the TechUtsav community.</p>
          
          <div class="feature-box">
            <h3>What's Next?</h3>
            <ul>
              <li>Explore exciting technical events and workshops</li>
              <li>Register for competitions and hackathons</li>
              <li>Connect with fellow tech enthusiasts</li>
              <li>Win amazing prizes and certificates</li>
            </ul>
          </div>
          ...
          <p style="margin-top: 30px;">We're excited to have you on board. If you have any questions, feel free to reach out to our support team.</p>
          
          <p style="font-weight: 600;">See you at the event! 🚀</p>
        </div>
        <div class="footer">
          <p>© ${new Date().getFullYear()} TechUtsav. All rights reserved.</p>
          <p>Need help? Contact us at support@techutsav.com</p>
        </div>
      </div>
    </body>
    </html>
...
export async function sendWelcomeEmail(email, name) {
  try {
    ...
    const info = await transporter.sendMail({
      from: `"TechUtsav Team" <${process.env.EMAIL_FROM}>`,
      to: email,
      subject: "Welcome to TechUtsav - Let's Get Started! 🎉",
      html: htmlContent,
      text: `Hi ${name}, Welcome to TechUtsav! Your registration is complete. Explore events, competitions, and connect with fellow tech enthusiasts. Visit ${process.env.NEXTAUTH_URL}/events to get started!`,
    });

    console.log(`[EMAIL] Welcome email sent successfully to ${email}. Message ID: ${info.messageId}`);
    return { success: true, messageId: info.messageId };
  } catch (error) {
